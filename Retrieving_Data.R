...
### view the working directory path
getwd()

### change working directory to filepath where datafile exists
setwd("C:/Users/hj163e/Documents/My Folder")
getwd()

### read .csv file
df <- read.csv("pain.csv", header = TRUE)
head(df)

### read .xlsx file
library(xlsx)
data <- read.xlsx("pain.xlsx",1)
head(data)
...
df <- sqlQuery(ch, "SELECT  * FROM ADW_TABLES_RAW.AER_AD SAMPLE 10")
df
